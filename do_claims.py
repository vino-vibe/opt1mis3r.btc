...
def run(account=None, live=False):
    ...
    for fp in fps:
        ...
        claims_path = CLAIMS_DIR / f"{fp.name}_claims_{today}.json"

        if not claims_path.exists():
            yield f"[{fp.name}] missing claims file: {claims_path}"
            yield ""
            continue
        ...
        sport_earnings = response_data.get("sportEarnings", [])
        if not sport_earnings:
            yield f"[{fp.name}] no sportEarnings in {claims_path}"
            yield ""
            continue

        yield f"[{fp.name}] processing {len(sport_earnings)} sports  live={live}"
        ...
        for sport_entry in sport_earnings:
            ...
            claimable = [pe for pe in pass_earnings if not pe.get("isDisabled", False)]
            if not claimable:
                yield f"[{fp.name}] {sport_name}: no claimable cards"
                continue
            claims_remaining = sport_entry.get("claimsRemaining", 0)
            cards_to_claim = claimable[:min(len(claimable), claims_remaining)]
            yield f"[{fp.name}] {sport_name}: claiming {len(cards_to_claim)} card(s)"
            ...
            for pe in cards_to_claim:
                ...
                label = pe.get("label", "unknown")
                if resp.status_code == 200:
                    yield f"  [{fp.name}] claimed {label} userPassId={pe['id']}"
                else:
                    try:
                        err = resp.json().get("message", resp.text)
                    except (json.JSONDecodeError, ValueError):
                        err = resp.text
                    yield (f"  [{fp.name}] FAILED {label} userPassId={pe['id']} "
                           f"status={resp.status_code} err={err}")
            yield ""
        yield ""
